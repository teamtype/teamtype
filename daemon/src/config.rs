...
impl AppConfig {
    #[must_use = "need to use function from_config_file()"]
    ...
    pub fn from_config_file(app_config_cli: Self) -> Self {
...
fn get_username(
    ...
    app_config_cli_username
        ...
        .or_else(get_username_from_fallback_value)
}

fn get_username_from_cli(username: String) -> String {
    info!(
        concat!(
            "Using the username '{}' from the flag `--username` ",
            "from when using the `share`/`join` subcommands as username, ",
            "to display next to the cursors other people see."
        ),
        username,
    );
    username
...
fn get_username_from_config_file(general_section: &Properties) -> Option<String> {
    general_section
        ...
        .map(|username| {
            info!(
                concat!(
                    "Using the username '{}' from `.teamtype/config` as username, ",
                    "to display next to the cursors other people see."
                ),
                username,
            );
...
fn get_username_from_git(base_dir: &Path) -> Option<String> {
    ...
    if let Some(ref username) = username {
        info!(concat!(
            "Using the Git username '{}' as username, ",
            "to display next to the cursors other people see.\n{}"),
            username,
            concat!(
...
fn get_username_from_fallback_value() -> Option<String> {
    let username = Some(USERNAME_FALLBACK.to_string());
    if let Some(ref username) = username {
        info!(concat!(
            "Using the fallback value for username '{}' as username, ",
            "to display next to the cursors other people see.\n{}"),
            username,
            concat!(
                "Teamtype uses the Git username as username by default.\n",
                "Using the fallback value instead can happen if Git is not installed,\n",
                "or if Git is installed, but no username is set on any level of Git configuration.\n",
                "You can set the configuration value `username` in your `.teamtype/config` to override this username.\n",
                "You can also use the flag `--username` when using the `share`/`join` subcommands."),
        );
    }
