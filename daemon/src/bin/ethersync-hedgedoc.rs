...
use teamtype::{
    ...
    path::RelativePath,
    types::{
        ComponentMessage, ContentLengthCodec, EditorTextDelta, RevisionedEditorTextDelta,
        ...
        match event.as_str() {
            "operation" => {
                if let Payload::Text(data) = data {
                    ...
                    let rev_delta = self.ot.apply_crdt_change(&delta).unwrap();
                    let delta = RevisionedTextDelta::try_from_rev_ed_delta(
                        rev_delta,
        ...
        match message {
            ComponentMessage::Edit { delta, .. } => {
                ...
                let rev_delta = RevisionedEditorTextDelta {
                    revision,
                    delta: EditorTextDelta::try_from_delta(
                        delta.clone(),
                        &self.ot.current_content(),
                    )
                    .unwrap(),
                ...
                let (delta_for_crdt, rev_deltas_for_editor) =
                    self.ot.apply_editor_operation(rev_delta).unwrap();
    ...
    > for EditorBinding
{
    ...
    fn handle_input_from_io(&mut self, message: EditorProtocolMessageFromEditor) {
        match message {
            ...
            EditorProtocolMessageFromEditor::Edit {
                ...
                let (delta_for_crdt, rev_deltas_for_editor) = self
                    .ot
                    .apply_editor_operation(rev_delta)
                    .expect("should work");
    ...
    fn handle_input_to_io(&mut self, message: ComponentMessage) {
        match message {
            ComponentMessage::Edit { delta, .. } => {
                let rev_delta = self.ot.apply_crdt_change(&delta).expect("should work");
                self.buffered_transmits_to_editor
    ...
    for AutoAcceptingJsonRpc
{
    ...
    fn handle_input_from_io(&mut self, message: String) {
        let parsed = IncomingMessage::from_jsonrpc(&message).expect("should work");
        let payload = match parsed {
            ...
            IncomingMessage::Notification { payload } => payload,
        };
