...
This release adds a couple of features and fixes some bugs. Read on for the highlights!

Special thanks to @alerque for putting in a lot of work behind the scenes to clean up various aspects of our code base.

## All relays are configurable now

It is now possible to specify custom Iroh relay servers, allowing you to use self-hosted ones.

For context: By default, Teamtype uses a couple of public relays hosted by other people/groups.
...
But if you don't want to rely on these public relays, you can now host your own ones, and instruct Teamtype to use them.
Using custom Magic Wormhole relays was already possible using the `--magic-wormhole-relay` flag.
In this release, we added support for custom Iroh relay servers, using the `--iroh-relay` flag, and for custom Iroh DNS discovery servers, using `--iroh-dns-domain` and `--iroh-pkarr-relay`.
See the [Running your own relays](https://teamtype.github.io/teamtype/relays.html) page in our documentation for how to run all these relays yourself, and for more information of how we use them.

## Prevent files from being shared using a `.teamtypeignore`

We added support for `.teamtypeignore` files to exclude files and directories from synchronization.
The syntax is similar to `.gitignore`, supporting file names, directories, and glob patterns.

Note that Teamtype also ignores the files specified in `.gitignore` files.
But `.teamtypeignore` files allow a more flexible way of specifying on which files you want to collaborate.

## Enable a status bar in the Neovim plugin

The Neovim plugin now exposes a `require("teamtype").status()` function, which returns a string summarizing information about the connection status and about remote cursors.
You can configure your status bar to display that information.

For example, if you're using [Lualine](https://github.com/nvim-lualine/lualine.nvim), you can use a configuration snippet like this:
...
## When Neovim plugin loses its connection to the daemon, make buffers read-only

When you have a shared file open in Neovim, but it loses connection to the daemon (because you stopped it, or because it crashed), the buffer will now be made read-only, to prevent accidental data loss.
You can then close the file, and reopen it when the daemon is running again.

## Policy change: No LLM contributions

We decided to adopt the following policy. It's been great!
...
If you're looking for inspiration for your own project, you can find a lot of other open-source AI contribution policies in [Melissa's list](https://github.com/melissawm/open-source-ai-contribution-policies).

## Other improvements in this release

- Fix an issue preventing directories with deep nesting (or very long winded names) from being shared.
- In the editor procotol, allow providing the JSON-RPC IDs as strings, to be compliant with the [JSON-RPC specification](https://www.jsonrpc.org/specification).
...
We restructured the location of the crates and of the main `Cargo.toml` file.
Instead of building Teamtype in the `daemon/` subdirectory, you now build it in the project root.
The position of the produced binary will be in `target/release`, and the manpage and completion files go directly to `target/`.
