...
Some things about Teamtype are currently still a bit annoying.
Let us show you how to work around them!

## Restarting the daemon requires restarting the editor

The editor plugins currently only try to connect to Teamtype when they first start.
If you need to restart the daemon for any reason, you will also need to restart all open editors to reconnect.
...
Then, edit the `.hex` file collaboratively.
Finally, convert back to a binary:
