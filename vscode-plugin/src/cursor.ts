...
let followedUserId: string | undefined = undefined;
...
export async function syncFollowCursor() {
    try {
        ...
        const cursorsToFollow = cursors.get(followedUserId);
...
export async function followCursor() {
    ...
    if (cursors.size === 1) {
        followedUserId = [...cursors.keys()][0];
        return;
    }
    cursors.forEach((remoteCursors, uid) => {
        if (remoteCursors.length > 0) {
            const label = `${followedUserId === uid ? "$(link) " : ""} ${remoteCursors[0].name}`;
