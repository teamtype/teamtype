...
const selectionDecorationType = vscode.window.createTextEditorDecorationType({
    ...
    before: {
        ...
        textDecoration:
            'font-weight: bold; position: absolute; top: 0; font-size: 200%; z-index: 0;',
    },
});
...
export function setCursor(
    ...
    name: string,
    uri: vscode.Uri,
    selections: vscode.DecorationOptions[]
) {
    ...
    if (usersCursors) {
        ...
        for (let cursor of usersCursors) {
            ...
            const editors = vscode.window.visibleTextEditors.filter(
                (editor) =>
                    editor.document.uri.toString() === cursor.uri.toString()
            );
