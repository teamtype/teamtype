...
class Client {
    ...
    constructor(name: string, cmd: string[], directory: string) {
        ...
        this.connection.onNotification('edit', (edit) =>
            processEditFromDaemon(this, edit)
        );
...
const openType = new rpc.RequestType<
...
function UTF16CodeUnitOffsetToCharOffset(
...
function charOffsetToUTF16CodeUnitOffset(
    charOffset: number,
    content: string
): number {
    let utf16Offset = 0;
    ...
    if (charOffset > chars.length) {
        throw new Error(
            `Could not convert char offset ${charOffset} to UTF-16 code unit offset in string '${content}'`
        );
    }
...
function vsCodeRangeToTeamtypeRange(
    content: string[],
    range: vscode.Range
): Range {
    return {
...
function vsCodePositionToTeamtypePosition(
    content: string[],
    ...
    return {
        line: position.line,
        character: UTF16CodeUnitOffsetToCharOffset(
            position.character,
            lineText
        ),
    };
...
function teamtypePositionToVSCodePosition(
    ...
    return new vscode.Position(
        position.line,
        charOffsetToUTF16CodeUnitOffset(position.character, lineText)
    );
}

function teamtypeRangeToVSCodeRange(
    document: vscode.TextDocument,
    range: Range
): vscode.Range {
...
function teamtypeDeltasToVSCodeTextEdits(
    document: vscode.TextDocument,
    deltas: Delta[]
): vscode.TextEdit[] {
...
export function contentAfterEdits(
    document: vscode.TextDocument,
    ...
    try {
        await mutex.runExclusive(async () => {
            ...
            if (edit.revision !== revision.editor) {
                debug(
                    `Received edit for revision ${edit.revision} (!= ${revision.editor}), ignoring`
                );
                return;
            }
            ...
            if (document) {
                let textEdits = teamtypeDeltasToVSCodeTextEdits(
                    document,
                    edit.delta
                );
                expectedContentAfterRemoteEdit = contentAfterEdits(
                    document,
                    textEdits
                );
                ...
                throw new Error(
                    `No document for URI ${uri}, why is the daemon sending me this?`
                );
            }
        });
    } catch (e) {
        vscode.window.showErrorMessage(
            `Error while processing edit from Teamtype daemon: ${e}`
        );
    }
}
...
function activateConfigForTextDocument(
    name: string,
    document: vscode.TextDocument,
    directory: string
) {
...
async function processUserOpen(document: vscode.TextDocument) {
    ...
    for (let name of Object.keys(configurations)) {
        ...
        for (let rootMarker of configuration.rootMarkers) {
            const directory = findMarkerDirectory(
        ...
        mutex
            .runExclusive(() => {
                ...
                for (const theEdit of edits) {
                    ...
                    client.connection
                        .sendRequest(editType, theEdit)
                        .catch((e) => {
                            vscode.window.showErrorMessage(
                                `Error while sending edit to Teamtype daemon: ${e}. This is probably a bug in the VS Code plugin.`
                            );
                        });
                    revision.editor += 1;

                    debug(
                        `sent edit for dR ${revision.daemon} (having edR ${revision.editor})`
                    );
                }
            ...
            .catch((e: Error) => {
                vscode.window.showErrorMessage(
                    `Error while sending edit to Teamtype daemon: ${e}`
                );
            });
    }
}
...
function vsCodeChangeEventToTeamtypeEdits(
    client: Client,
...
function vsCodeChangeToTeamtypeDelta(
    content: string[],
...
export function activate(context: vscode.ExtensionContext) {
    ...
    configurations = vscode.workspace
        .getConfiguration('teamtype')
        .get<{ [key: string]: Configuration }>('configs', {});
