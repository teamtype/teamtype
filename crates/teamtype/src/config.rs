...
impl Config {
    ...
    pub async fn resolve_peer(self, ui: &UserInterface) -> Result<Self> {
        if let Some(peer) = self.peer.clone() {
            ...
            let resolved_peer = match peer {
                Peer::JoinCode(ref join_code) => {
                    ...
                    store_peer_in_config(self, &secret_address, ui)?;
            ...
            result.peer = Some(resolved_peer);
            Ok(result)
        } else {
            // In share mode, there is no peer to even possibly need resolving.
            Ok(self)
        }
    }
...
fn store_peer_in_config(config: Config, peer: &str, ui: &UserInterface) -> Result<()> {
