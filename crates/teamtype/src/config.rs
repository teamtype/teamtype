...
impl AppConfig {
    ...
    pub fn from_config_file_and_cli(app_config_cli: Self, ui: Interface) -> Self {
...
fn get_username(
    ...
    app_config_cli_username
        .map(|u| get_username_from_cli(u, ui.clone()))
        .or_else(|| get_username_from_config_file(general_section, ui.clone()))
        .or_else(|| get_username_from_git(base_dir, ui.clone()))
        .unwrap_or_else(|| get_username_from_fallback_value(ui))
}

fn get_username_from_cli(username: String, ui: Interface) -> String {
...
fn get_username_from_config_file(general_section: &Properties, ui: Interface) -> Option<String> {
    general_section
        .get("username")
        .map(ToString::to_string)
        .map(|username| {
...
fn get_username_from_git(base_dir: &Path, ui: Interface) -> Option<String> {
...
fn get_username_from_fallback_value(ui: Interface) -> String {
