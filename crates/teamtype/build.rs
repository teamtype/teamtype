...
fn main() -> Result<()> {
    ...
    let version = derive_and_pass_version_details()?;
...
fn derive_and_pass_version_details() -> Result<&'static str> {
...
fn formulate_version() -> Result<&'static str> {
    ...
    let dirty_suffix = matches!(is_dirty.as_ref(), "true")
        .then_some("-dirty")
        .unwrap_or("");
    let version = format!("{semver} ({}{dirty_suffix} {commit_date})", &git_sha[..9]);
